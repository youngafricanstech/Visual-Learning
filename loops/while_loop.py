...
def visualize_while_loop(variable_name, variable_value, comparison_operator, target_value):
    ...
    increment_text = font.render(f"{variable_name} += {increment_value}", True, (255,255,0))
    screen.blit(increment_text, (260, 220))
...
def run_while_loop():
    ...
    # Validate the increment operator format
    if not validate_increment_format(increment_operator_str):
        messagebox.showerror("Validation Error", "Invalid increment operator format. Please use 'counter += 1' or 'counter += 2' format.")
        return

    # Extract the increment value using regex
    match_increment = re.search(r"counter\s*\+=\s*(\d+)", increment_operator_str)
    if match_increment:
        global increment_value
        increment_value = int(match_increment.group(1))
    else:
        messagebox.showerror("Validation Error", "Invalid increment operator. Please use 'counter += 1' or 'counter += 2' format.")
        return
    ...
    def run_while_loop_thread():
        ...
        while eval(f"counter {comparison_operator} target_value - 1"):
            exec(increment_operator_str, None, variables)
            counter = variables.get("counter", 0)
            visualize_while_loop("counter", counter, comparison_operator, target_value)
            time.sleep(2)
...
# Function to validate the increment operator format
def validate_increment_format(increment_str):
    pattern = re.compile(r"counter\s*\+=\s*\d+")
    return bool(pattern.match(increment_str))
...
increment_entry = Text(root, font=("Arial", 16), width=26, height=1)
